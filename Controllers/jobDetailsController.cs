...
namespace NewLetter.Controllers
{
    public class jobDetailsController : BaseClass
    {
        ...
        public ActionResult Jobsearch(FormCollection frm)
        {
            ...
            var jobDetails = db.Database.SqlQuery<sp_searchjob_Result>(query).ToList();
        ...
        public ActionResult JobView(string jobID)
        {
            ...
            if (jobid_ != 0)
            {
                var jobDetails = db.jobDetails.Include(e => e.industry).Include(e => e.EmploymentType).Include(e => e.companyDetail).Where(e => e.jobID == jobid_).FirstOrDefault();
        ...
        public JsonResult select_fillSkills(string term)
        {
            ...
            var skillName = db.skills.Where(m => m.skillName.Contains(term)).Select(x => new { value = x.skillName, userId = x.skillsID }).Distinct().Take(10);
        ...
        public ActionResult companyDetails_(string  CompanyID)
        {
            ...
            var t = Json(ocompanyD, JsonRequestBehavior.AllowGet);
            return t;
        }
        ...
        public ActionResult adPostSuccess(long ID)
        {
            ...
            //var addetails = db.advertisementLists.Where(e => e.advertisementRefID == ID).FirstOrDefault();
            var adlist = db.advertisementLists.Where(e => e.advertisementRefID == ID).FirstOrDefault();
            ViewBag.refID = ID;
            return View(adlist);
        }
        ...
        {
            var advert = db.advertisementLists.Where(a => a.advertisementRefID == ID).FirstOrDefault();
            ViewBag.ID = ID;
            return View(advert);
        }
        ...
        public ActionResult MyJobs()
        {
            ...
            var jobs = db.jobDetails.Where(e => e.companyID == companyID && e.compeletPosted == true).OrderByDescending(e=>e.dataIsCreated).ToList();
            return View(jobs);
        }
        ...
        {            
            int companyID = Convert.ToInt32(BaseUtil.GetSessionValue(AdminInfo.companyID.ToString()));
            var jobs = db.jobDetails.Where(e => e.companyID == companyID && e.compeletPosted == false).OrderByDescending(e => e.dataIsCreated).ToList();
            return View(jobs);
        }
